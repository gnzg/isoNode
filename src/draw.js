...
export default function draw (canvas, mapsObject, mapXparam, mapYparam, tileWidthParam, z, x, y, rectColorsParam, rectShadowColorsParam) {
  ...
  if (
    ...
    if (k === 1 && i === 0 && j === 0) {
      ...
      let gimel = Math.sqrt(Math.pow(tileWidth * i + tileWidth + mapX + tileWidth * j, 2) + Math.pow(tileWidth + topYsegment));
      let daled = Math.sqrt(Math.pow(tileWidth * i + tileWidth * 2 + mapX + tileWidth * j, 2) + Math.pow( d + topYsegment));
      // save the tile's vector magnitudes, i.e. hitbox boundries
      state.env.tileHitBoxes[0] = [alef, beth, gimel, daled];
    }
