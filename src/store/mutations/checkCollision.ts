  ...
  for (let i = 0; i < tileCoordinates.length; i++) {
    ...
    if (pointInRhombus(tileCoordinates[i], { x: cursor_pos_x, y: cursor_pos_y })) {
      ...
      // or, if currentlyHoveredTile does not correspond to the current tile coordinates, update the former
        ...
        store.dispatch("saveLastHoveredTile", store.state.map_data.currentlyHoveredTile);
        store.dispatch("onTileHover", store.state.map_data.currentlyHoveredTile);
        store.dispatch("updateCanvas");
      
        // if currentlyHoveredTile is different than lastHoveredTile, unhover the former
      if (store.state.map_data.lastHoveredTile.x != tileCoordinates[i].x) {
        store.state.map_data.currentlyHoveredTile = {
          x: tileCoordinates[i].x,
          y: tileCoordinates[i].y,
        };
